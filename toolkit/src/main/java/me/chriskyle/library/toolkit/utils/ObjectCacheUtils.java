...
import me.chriskyle.library.toolkit.coder.MD5Coder;

/**
 * 数据/配置存储类
 */
public final class ObjectCacheUtils {

    public static final String DEFAULT_CACHE_NAME = "appCache";

    private File mCacheFile;
    private static Map<String, ObjectCacheUtils> mCacheUtilsMap = new HashMap<>();

    private ObjectCacheUtils(File cacheFile) {
    ...
    public static ObjectCacheUtils getInstance(Context ctx) {
        return getInstance(new File(StorageUtils.getCacheDirectory(ctx), DEFAULT_CACHE_NAME));
    }

    public static ObjectCacheUtils getInstance(String path, String cacheDirName) {
        return getInstance(new File(path, cacheDirName));
    }

    public static ObjectCacheUtils getInstance(Context ctx, String cacheDirName) {
        return getInstance(new File(StorageUtils.getCacheDirectory(ctx), cacheDirName));
    }

    public static ObjectCacheUtils getInstance(File file) {
        ObjectCacheUtils objectCacheUtils = mCacheUtilsMap.get(file.getAbsolutePath());
        if (objectCacheUtils == null) {
            objectCacheUtils = new ObjectCacheUtils(file);
            mCacheUtilsMap.put(file.getAbsolutePath(), objectCacheUtils);
        }
        return objectCacheUtils;
    }
